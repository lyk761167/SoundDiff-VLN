...
from ss_baselines.savi.ppo.sounddiff_policy import SoundDiffVLNPolicy
...
class PPOTrainer(BaseRLTrainer):
    ...
    def __init__(self, config=None):
        ...
        self._belief_enabled = False

       
        self._sounddiff_mode = False
        self._sounddiff_state_dim = None   
        self._rollout_hidden_size = None   

    def _policy_in_hidden(self, h: torch.Tensor) -> torch.Tensor:
        """
        rollout/eval 里可能是 [L,B,H] 或 [B,H]。
        SoundDiffNet 需要 [B, H+M*D]。
        """
        if not self._sounddiff_mode:
            return h
    ...
    def _policy_out_hidden(self, h_out: torch.Tensor, h_ref: torch.Tensor) -> torch.Tensor:
        """
        policy/net 输出可能是 [B,D]（SoundDiff），但 rollouts 里可能存 [L,B,D] 或 [B,D]。
        这里对齐到 h_ref 的形状。
        """
        if not self._sounddiff_mode:
            return h_out
        ...
        if h_ref.dim() == 3 and h_out.dim() == 2:
            
            return h_out.unsqueeze(0)

     
        if h_ref.dim() == 2 and h_out.dim() == 3 and h_out.size(0) == 1:
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, observation_space=None) -> None:
        ...
        policy_type = getattr(ppo_cfg, "policy_type", "interactive")


        if policy_type == "sounddiff_vln":
            self._sounddiff_mode = True

     
            if getattr(ppo_cfg, "use_external_memory", False):
                logger.warning(
                    "[sounddiff_vln] 当前 SoundDiffVLNNet 将 memory 打包进 recurrent_hidden_states，"
                    "不兼容 Trainer 的 ExternalMemory 路径。将强制关闭 use_external_memory。"
                )
                self.config.defrost()
                self.config.RL.PPO.use_external_memory = False
                self.config.freeze()
                ppo_cfg.use_external_memory = False

           
            if getattr(ppo_cfg, "use_belief_predictor", False):
                logger.warning(
                    "[sounddiff_vln] BeliefPredictor 依赖 smt_state_encoder，不兼容 SoundDiffVLNPolicy。"
                    "将强制关闭 use_belief_predictor 以避免报错。"
                )
                self.config.defrost()
                self.config.RL.PPO.use_belief_predictor = False
                self.config.freeze()
                ppo_cfg.use_belief_predictor = False

            self.actor_critic = SoundDiffVLNPolicy(
            ...
            net = self.actor_critic.net
            try:
                self._sounddiff_state_dim = int(net.rnn_hidden_size + net.mem_slots * net.embed_dim)
            except Exception as e:
                raise RuntimeError(
                    "[sounddiff_vln] 无法从 net 计算 rnn state dim。"
                    "请确保 net 有 rnn_hidden_size/mem_slots/embed_dim 属性。"
                ) from e
            self._rollout_hidden_size = self._sounddiff_state_dim

     
        else:
            self._sounddiff_mode = False
            self._sounddiff_state_dim = None
            self._rollout_hidden_size = ppo_cfg.hidden_size
        ...
        if self.config.RESUME:
            ...
            def _safe_load(module, keyname: str):
                if module is None:
                    return
                try:
                    sd = self.search_dict(ckpt_dict, keyname)
                    if len(sd) > 0:
                        module.load_state_dict(sd, strict=False)
                except Exception as e:
                    logger.warning(f"Resume load for {keyname} skipped: {e}")

            net = self.agent.actor_critic.net
            _safe_load(getattr(net, "visual_encoder", None), "visual_encoder")
            _safe_load(getattr(net, "goal_encoder", None), "goal_encoder")
            _safe_load(getattr(net, "action_encoder", None), "action_encoder")

       
        if getattr(ppo_cfg, "use_external_memory", False) and hasattr(ppo_cfg, "SCENE_MEMORY_TRANSFORMER"):
            smt_cfg = ppo_cfg.SCENE_MEMORY_TRANSFORMER
            if ppo_cfg.use_external_memory and getattr(smt_cfg, "freeze_encoders", False) and hasattr(self.actor_critic.net, "freeze_encoders"):
    ...
    def try_to_resume_checkpoint(self):
        ...
        if len(checkpoints) == 0:
            ...
            if self._belief_enabled and "belief_predictor" in ckpt_dict:
    ...
    def _collect_rollout_step(self, rollouts, current_episode_reward, running_episode_stats):
        ...
        with torch.no_grad():
            ...
            h_ref = rollouts.recurrent_hidden_states[rollouts.step]
            h_in = self._policy_in_hidden(h_ref)
            ...
            recurrent_hidden_states = self._policy_out_hidden(recurrent_hidden_states, h_ref)
        ...
        for k, v in self._extract_scalars_from_infos(infos).items():
            v = torch.tensor(v, dtype=torch.float, device=current_episode_reward.device).unsqueeze(1)
    ...
    def train_belief_predictor(self, rollouts):
        ...
        for e in range(num_epoch):
            data_generator = rollouts.recurrent_generator(advantages, num_mini_batch)
            ...
            for sample in data_generator:
                ...
                masks = (torch.sum(torch.reshape(
                    obs_batch[SpectrogramSensor.cls_uuid],
                    (obs_batch[SpectrogramSensor.cls_uuid].shape[0], -1)),
                    dim=1, keepdim=True
                ) != 0).float()
                ...
                bitwise_close = torch.bitwise_and(
                    torch.isclose(rounded_preds[:, 0], transformed_gts[:, 0]),
        ...
        prediction_accuracy = 0 if num_sample == 0 else running_regressor_corrects / num_sample
    ...
    def train(self) -> None:
        ...
        # ✅ 关键：RolloutStorage hidden_size 用 override（sounddiff 用 H+M*D）
        hidden_size_for_rollout = int(self._rollout_hidden_size or ppo_cfg.hidden_size)
        ...
        if interrupted_state is not None:
            ...
            self.agent.optimizer.load_state_dict(interrupted_state["optimizer_state"])
    ...
    def _eval_checkpoint(self, checkpoint_path: str, writer: TensorboardWriter, checkpoint_index: int = 0) -> Dict:
        ...
        hidden_size_for_eval = int(self._rollout_hidden_size or ppo_cfg.hidden_size)
        ...
        while (len(stats_episodes) < self.config.TEST_EPISODE_COUNT and self.envs.num_envs > 0):
            ...
            with torch.no_grad():
                h_ref = test_recurrent_hidden_states
                h_in = self._policy_in_hidden(h_ref)

                _, actions, _, h_out, test_em_features = self.actor_critic.act(
                    batch,
                    h_in,
                    prev_actions,
                    not_done_masks,
                ...
                h_out = self._policy_out_hidden(h_out, h_ref)
                test_recurrent_hidden_states = h_out
            ...
            for i in range(self.envs.num_envs):
                if len(self.config.VIDEO_OPTION) > 0:
                    pred = descriptor_pred_gt[i][-1] if self._belief_enabled else None
            ...
            rewards_t = torch.tensor(rewards, dtype=torch.float, device=self.device).unsqueeze(1)
            current_episode_reward += rewards_t
            ...
            for i in range(self.envs.num_envs):
                ...
                if not_done_masks[i].item() == 0:
                    ...
                    if "top_down_map" in self.config.VISUALIZATION_OPTION:
                        pred = episode_stats['descriptor_pred_gt'][-1] if self._belief_enabled else None
                        ...
                        sound = current_episodes[i].sound_id.split('/')[1][:-4]
                        writer.add_image(
                            f"{config.EVAL.SPLIT}_{scene}_{current_episodes[i].episode_id}_{sound}/{infos[i]['spl']}",
                            top_down_map,
                            dataformats='WHC'
                        )
        ...
        stats_file = os.path.join(
            config.TENSORBOARD_DIR, '{}_stats_{}.json'.format(config.EVAL.SPLIT, config.SEED)
        )
