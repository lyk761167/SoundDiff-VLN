...
class VisualCNN(nn.Module):
    ...
    def forward(self, observations):
        ...
        # 把末尾多余的 size=1 维度都 squeeze 掉，直到 <=4 维 或者最后一维不是 1
            while depth_observations.dim() > 4 and depth_observations.shape[-1] == 1:
               depth_observations = depth_observations.squeeze(-1)

        # 理想情况现在应该是 [B, H, W, C]
            if depth_observations.dim() != 4:
               raise RuntimeError(
                f"Expected depth observations to be 4D [B,H,W,C], "
                f"but got shape {depth_observations.shape}"
                )
